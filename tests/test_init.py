...
async def test_setup_unload_and_reload_entry(hass, bypass_outputs_get_all):
    ...
    config_entry = MockConfigEntry(domain=DOMAIN, data=LOCALGW_MOCK_CONFIG, entry_id="test")

    assert await async_setup_entry(hass, config_entry)
    assert DOMAIN in hass.data and config_entry.entry_id in hass.data[DOMAIN]
    assert (
        type(hass.data[DOMAIN][config_entry.entry_id]) == OpenMoticsLocalDataUpdateCoordinator
    )
