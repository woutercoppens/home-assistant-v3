...
from custom_components.openmotics import (
    async_setup_entry,
    async_unload_entry,
)
from custom_components.openmotics.const import DOMAIN
from custom_components.openmotics.coordinator import OpenMoticsCloudDataUpdateCoordinator

from .const import CLOUD_MOCK_CONFIG
...
async def test_setup_unload_and_reload_entry(hass):
    ...
    config_entry = MockConfigEntry(domain=DOMAIN, data=CLOUD_MOCK_CONFIG, entry_id="test")
    ...
    assert isinstance(
        hass.data[DOMAIN][config_entry.entry_id], OpenMoticsCloudDataUpdateCoordinator
    )
